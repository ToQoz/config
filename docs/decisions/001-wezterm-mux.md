...
- Neovim で `Cmd-p` のような keymap を設定したい
- Neovim (0.11+) は Kitty keyboard protocol (`CSI u`) を experimental support
- WezTerm は Kitty keyboard protocol を扱え、built-in mux もある

## Decision

terminal multiplexer は WezTerm の mux を使いながらも、これまでと同じような操作感で使えるようにに設定する
...
Command を修飾キーとした keymap が定義できることで、使いやすく衝突しにくい定義をしやすくなった。
