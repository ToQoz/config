...
in
{
  ...
  window-picker-happy = mkCheck "window-picker-happy" windowPicker ''
    ...
    diff -u <(printf '%s\n' \
      "tcmux list-windows -A --color=always -F #{session_name}:#{window_index}: #{window_name} (#{window_panes} panes) #{agent_status}" \
      "fzf --sync --ansi --layout reverse --tmux 80%,50% --bind start:pos(2)" \
      "tmux select-window -t default:1") "$TMUX_FAKE_LOG"
  '';
  ...
  window-picker-no-current-match = mkCheck "window-picker-no-current-match" windowPicker ''
    ...
    diff -u <(printf '%s\n' \
      ...
      "tmux select-window -t default:0") "$TMUX_FAKE_LOG"
  '';
}
